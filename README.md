...
Entrance selections are saved as export data; optional chunks restrict an entrance once configured. A unique boss-specific cave/lair or explicit arena location preselects arena coverage for a new draft. Broad or ambiguous locations stay unselected. Saved drafts and existing plugin regions are preserved. New drafts default to Unsafe death; existing classifications are preserved. Export validates the configuration without requiring verification checkboxes. The editor keeps arena selections separate, while previews, patches, proposals, and PR generation combine arena and configured entrance regions into the existing Java region list. Entrance chunk regions are included automatically. If arena chunk restrictions are enabled, explicit entrance chunks are required and merged into the exported restriction list. Missing locations and conflicting selections block export with an actionable message.
