...
export function RegionMap(props:Props){
 ...
 const [fine,setFine]=useState(false),[missing,setMissing]=useState(false),[zoom,setZoom]=useState(1),[workerMode,setWorkerMode]=useState(false);
 const fineRef=useRef(false);fineRef.current=fine;const readout=useRef<HTMLElement>(null);
 useEffect(()=>{
  if(!host.current)return;
  const m=L.map(host.current,{crs:L.CRS.Simple,minZoom:0,maxZoom:5,zoomSnap:.25,zoomDelta:.5,wheelPxPerZoomLevel:100,wheelDebounceTime:30,zoomControl:false,attributionControl:true,preferCanvas:true}).setView([props.center[1],props.center[0]],1);
 ...
 useEffect(()=>{setMissing(false);imagery.current?.redraw();},[props.tileSource,props.plane]);
 ...
 return <div className="map-wrap" data-renderer={workerMode?'worker':'canvas'}><div ref={host} className="map" aria-label="OSRS region selection map"/><div className="map-note">{props.chunkMode?'Click an 8×8 chunk':'Click a region'} · Drag to pan · Scroll to zoom</div><div className="map-bottom"><code ref={readout}>Explore the map</code><button onClick={()=>setFine(v=>!v)} aria-pressed={fine}>{fine?'Hide':'Show'} grid</button><span>{zoom.toFixed(2)}×</span></div>{missing&&<div className="tile-note">Some tiles unavailable · selection still works</div>}</div>;
}
