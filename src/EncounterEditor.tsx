...
import {generateEntry} from './core/java.mjs';
...
function values(draft:Draft, section:Section):[string,string][] {
  ...
  return [['Priority',knownPriority ? priority(entranceOverlay(current)) : 'Preserved in source'],
...
export default function EncounterEditor(p:Props) {
  const baseline = useMemo(()=>editingBaseline(p.draft.baseRaw),[p.draft.baseRaw]);
  const sourceBoss = useMemo(()=>({...p.boss,...baseline.entry}) as Boss,[p.boss,baseline]);
  ...
  function apply() {
    ...
    try {
      ...
      validateProposal({version:1,repository:PLUGIN_REPO,baseCommit:'0'.repeat(40),changes:[next]});
      generateEntry(next,baseline.entry);
  ...
  const entranceChunks:number[]=effective.entrance?.chunks??originalEntranceChunks(baseline.entry.optionalArgs);
  const entranceRegions=entranceChunks.length
    ? [...new Set(entranceChunks.map((id:number)=>{const point=chunkOrigin(id);return regionId(point.x,point.y);}))]
    : effective.entranceRegion!==undefined?[effective.entranceRegion]:(effective.entrance||baseline.entrance.raw)?effective.regions:[];
  const preview = (()=>{try{return generateEntry(p.draft,baseline.entry)+',';}catch(e){return (e as Error).message;}})();
    ...
    {section&&working ? <div className="edit-session">
      ...
      <div className="edit-overview-layout"><div className="edit-records">{sections.map(id=>{
        ...
        return <section key={id} className={`edit-record ${modified?'is-modified':''} ${collapsed[id]?'is-collapsed':''}`}><div className="edit-record-heading"><h3><button className="edit-collapse-toggle" aria-expanded={!collapsed[id]} aria-controls={`overview-${id}`} onClick={()=>setCollapsed(previous=>({...previous,[id]:!previous[id]}))}><span className="edit-collapse-chevron" aria-hidden="true">›</span><Icon name={id==='details'?'file':id==='coverage'?'map':'layers'} size={19}/>{labels[id]}</button>{modified&&<span className="edit-change-label">Changed</span>}</h3><button data-edit-section={id} disabled={!!blocked} onClick={()=>open(id)}>{id==='entrance'&&!baseline.entrance.raw&&!effective.entrance?'Add entrance':`Edit ${labels[id].toLowerCase()}`} <Icon name="arrow" size={14}/></button></div>
          ...
          {!collapsed[id]&&id==='entrance'&&entranceRegions.length>0&&<OverviewMapSquares regions={entranceRegions} chunks={entranceChunks} locations={sourceBoss.maps.filter(location=>location.role==='entrance')} context={p.context.entrance} label="Entrance"/>}
      ...
      {review&&<div className="edit-review-evidence">{overlapWarnings([p.draft],p.snapshot.entries).map((warning:string)=><p className="warning" key={warning}>{warning}</p>)}<details><summary>Compare generated code</summary><h3>Original plugin entry</h3><pre>{p.draft.baseRaw}</pre><h3>Your draft</h3><pre>{preview}</pre></details></div>}
    </>}
