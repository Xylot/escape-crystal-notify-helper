...
import { loadGameval, searchGameval, gamevalMatches, indexGameval } from './core/gameval.mjs';
import {loadRegionObjects,entranceObjectRegions,entranceObjectCandidates,objectsInEntranceChunks} from './core/region-objects.mjs';
import { Icon } from './Icons';
import RegionObjects from './RegionObjects';
import {originalEntranceChunks} from './core/encounter.mjs';
import {selectEntranceIds} from './core/entrance-selection.mjs';
import GamevalNames, {useGamevalNames} from './GamevalNames';
...
export default function EntranceEditor({ boss, draft, update, onError, guided = false, editing = false, imageSelection, onImageSelect, onEditArea }: {
  ...
  const edit = (part: Partial<Config>) => update(part.ids || part.objectType ? selectEntranceIds(draft,config,part.ids??config.ids,part.objectType??config.objectType) : { entrance: { ...config, ...part } });
  const addIds = async (values: string[], objectType = 'GAME_OBJECT', placement?:{x:number;y:number}) => {
    const next = [...new Set([...(config.objectType === objectType ? config.ids : []), ...values])];
    let placements:Record<string,{x:number;y:number}[]>=placement?Object.fromEntries(values.map(id=>[id,[placement]])):{};
    if(!placement&&objectType==='GAME_OBJECT'&&next.length){
      try{
        const regions=entranceObjectRegions(draft.entranceRegion,draft.entranceNotifyChunks??[]);
        if(regions.length){
          const data=await loadRegionObjects();
          const rows=objectsInEntranceChunks(entranceObjectCandidates(data,regions,[],draft.entrancePlane??0),draft.entranceNotifyChunks??[]);
          for(const id of next){
            const numeric=/^\d+$/.test(id)?[id]:gamevalMatches(gameval.index??indexGameval(await loadGameval()),[id],'GAME_OBJECT').map((entry:{id:string})=>entry.id);
            const matches=rows.filter(row=>numeric.includes(String(row.id))||row.forms.some((form:{locId:number})=>numeric.includes(String(form.locId))));
            if(matches.length)placements[id]=matches;
          }
        }
      }catch(error){onError(`Could not locate object chunks: ${(error as Error).message}. The selected notification area is used for IDs without a known location.`);}
    }
    if(currentDraft.current!==draft)return;
    setIds(next.join(', ')); update(selectEntranceIds(draft,config,next,objectType,placements));
  };
  ...
  return <div className="entrance-object-studio">
    ...
    {gameval.error?<p className="export-issue" role="status">RuneLite gameval names unavailable. {gameval.error} <button type="button" onClick={gameval.retry}>Retry gameval lookup</button></p>:!gameval.index&&<p className="muted" role="status">Loading RuneLite gameval names…</p>}
