...
export default function EntranceEditor({ boss, draft, update, onError, guided = false, editing = false, imageSelection, onImageSelect, onEditArea }: {
  ...
  return <div className="entrance-object-studio">
    ...
    <div hidden={method !== 'region'}><RegionObjects selectedChunks={draft.entrance?.chunks??originalEntranceChunks(boss.optionalArgs)} gameval={gameval} lockedArea boss={boss} selectedRegion={draft.entranceRegion} selectedPlane={draft.entrancePlane} selectedIds={config.objectType === 'GAME_OBJECT' ? config.ids : []} autoLoad={guided && method === 'region'} onError={onError} onUse={id => addIds([id])}/></div>
