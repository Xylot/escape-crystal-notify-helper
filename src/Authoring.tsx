...
export default function Authoring(p: Props) {
  ...
  const readOnly = (!!boss.raw && !['BOSSES','RAIDS','DUNGEONS'].includes(boss.regionType??'')) || (!boss.raw && !!boss.supportedBy?.length);
  ...
  const steps: { id: AuthoringStep; label: string; description: string; done: boolean }[] = dungeon ? [
    ...
    { id: 'setup', label: 'Setup', description: 'Four quick questions', done: draft.setupProgress===4 && !!draft.recommendedSeconds && !!draft.petIcon },
  ...
  return <section className={`authoring ${dungeon?'dungeon-authoring':''}`} aria-label="Encounter authoring" data-step={step}>
    ...
    {step === 'setup' ? <fieldset disabled={readOnly} className="authoring-fields"><EncounterSetup boss={p.boss} draft={draft} update={p.update} onContinue={()=>setStep('coverage')}/></fieldset> : step === 'entrance-object' ? <fieldset disabled={readOnly} className="authoring-fields object-workspace"><EntranceEditor imageSelection={p.evidenceContext?.entranceImage} onImageSelect={p.onEntranceImage} boss={boss} draft={draft} update={p.update} onError={p.onError} onEditArea={()=>setStep('entrance-area')} guided/></fieldset> : step !== 'review' ? <div className={`authoring-body ${step === 'entrance-area' ? (choosingEntranceMap?'entrance-step choosing-entrance-map':'entrance-step') : ''}`}>
      ...
      <section className="review-summary-card"><div><Icon name="map"/><h3>{dungeon?'Dungeon coverage':'Arena regions'}</h3><button onClick={() => setStep('coverage')}>Edit coverage</button></div><dl><dt>Regions</dt><dd>{draft.regions.join(', ') || 'None selected'}</dd><dt>Death behavior {!dungeon&&<>· <button onClick={()=>setStep('setup')}>Edit setup</button></>}</dt><dd>{draft.deathType === 'UNSAFE' ? 'Unsafe death' : draft.deathType === 'UNSAFE_HCGIM' ? 'Unsafe for Hardcore Group Ironman' : draft.deathType || 'Not selected'}</dd>{!dungeon&&<><dt>Recommended inactivity time</dt><dd>{draft.recommendedSeconds==null?'Not selected':`${draft.recommendedSeconds} seconds`}</dd><dt>Pet icon</dt><dd>{draft.petIcon||'Not selected'}</dd></>}<dt>Chunk restrictions</dt><dd>{draft.chunks?.length ? draft.chunks.join(', ') : 'Whole regions'}</dd></dl></section>
      {!dungeon&&<><section className="review-summary-card"><div><Icon name="map"/><h3>Entrance regions</h3><button onClick={()=>setStep('entrance-area')}>Edit area</button></div><dl><dt>Regions</dt><dd>{coverage?.entranceRegions.join(', ') || draft.entranceRegion || 'Not selected'}</dd><dt>Plane</dt><dd>{draft.entrancePlane ?? p.evidenceContext?.entrance?.plane ?? 'Not selected'}</dd><dt>Boss fight</dt><dd>{draft.bossInstanced?'Instanced · region notifications only inside the instance':'Not restricted to an instance'}</dd><dt>Entrance area</dt><dd>{exported.entranceDangerous===false?'Not dangerous · region notifications disabled':exported.entranceDangerous===true?'Dangerous':'Existing behavior preserved'}</dd><dt>Entrance coverage chunks</dt><dd>{exported.entranceNotifyChunks?.join(', ')||'Whole entrance area'}</dd><dt>Object-detection chunks</dt><dd>{entranceChunks.join(', ') || 'No restriction'}</dd></dl></section>
    ...
    <footer className="authoring-footer"><div><span className="dot mint-dot"/><span>{p.saved ? 'Your changes are saved locally.' : 'Changes save automatically as you edit.'}</span></div><div>
      ...
      {step !== 'review' && step !== 'setup' && !(step==='entrance-area'&&choosingEntranceMap) && <button className="primary" onClick={()=>setStep(steps[steps.findIndex(s=>s.id===step)+1].id)}>{dungeon?'Review & export':step==='coverage'?'Continue to entrance area':step==='entrance-area'?'Choose entrance object':'Review & export'} <Icon name="arrow" size={16}/></button>}
