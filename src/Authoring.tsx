...
export default function Authoring(p: Props) {
  ...
  const issue = useMemo(()=>step==='review'?exportProblem(p.snapshot, [draft]):'', [step,p.snapshot,draft]);
  let coverage:ReturnType<typeof encounterCoverage>|undefined;
  try { if(step==='review')coverage = encounterCoverage(draft, boss.raw ? boss : null); } catch { /* The export error is displayed below. */ }
  const entranceChunks = draft.entrance?.chunks ?? originalEntranceChunks(boss.optionalArgs);
  const warnings = useMemo(()=>step==='review'?overlapWarnings([draft], p.snapshot.entries):[],[step,draft,p.snapshot.entries]);
