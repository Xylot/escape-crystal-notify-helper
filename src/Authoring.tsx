...
function EntranceChunks({draft,update,onError,preserved}:{draft:Draft;update:Props['update'];onError:Props['onError'];preserved:boolean}) {
  ...
  return <details className="inspector-disclosure"><summary>Enter chunk IDs</summary><label className="field">Entrance chunk IDs<input ref={input} value={text} placeholder="No chunk restriction" onChange={e=>{
    ...
    update({entrance:{overlay:entranceOverlay(draft),direction:'',plane:'',objectType:'GAME_OBJECT',ids:[],...draft.entrance,chunks}});
  }}/></label></details>;
}
...
export default function Authoring(p: Props) {
  ...
  return <section className="authoring" aria-label="Encounter authoring" data-step={step}>
    ...
    {step === 'setup' ? <fieldset disabled={readOnly} className="authoring-fields"><EncounterSetup draft={draft} update={p.update}/></fieldset> : step === 'entrance-object' ? <fieldset disabled={readOnly} className="authoring-fields object-workspace"><EntranceEditor imageSelection={p.evidenceContext?.entranceImage} onImageSelect={p.onEntranceImage} boss={boss} draft={draft} update={p.update} onError={p.onError} onEditArea={()=>setStep('entrance-area')} guided/></fieldset> : step !== 'review' ? <div className={`authoring-body ${step === 'entrance-area' ? 'entrance-step' : ''}`}>
      ...
      <section className="review-summary-card"><div><Icon name="map"/><h3>Arena coverage</h3><button onClick={() => setStep('coverage')}>Edit coverage</button></div><dl><dt>Regions</dt><dd>{draft.regions.join(', ') || 'None selected'}</dd><dt>Death behavior · <button onClick={()=>setStep('setup')}>Edit setup</button></dt><dd>{draft.deathType === 'UNSAFE' ? 'Unsafe death' : draft.deathType === 'UNSAFE_HCGIM' ? 'Unsafe for Hardcore Group Ironman' : draft.deathType || 'Not selected'}</dd><dt>Chunk restrictions</dt><dd>{draft.chunks?.length ? draft.chunks.join(', ') : 'Whole regions'}</dd></dl></section>
    ...
    <footer className="authoring-footer"><div><span className="dot mint-dot"/><span>{p.saved ? 'Your changes are saved locally.' : 'Changes save automatically as you edit.'}</span></div><div>
      {step !== 'setup' && <button onClick={()=>setStep(steps[steps.findIndex(s=>s.id===step)-1].id)}>← Back</button>}
      ...
      {step !== 'review' && <button className="primary" onClick={()=>setStep(steps[steps.findIndex(s=>s.id===step)+1].id)}>{step==='setup'?'Continue to arena':step==='coverage'?'Continue to entrance area':step==='entrance-area'?'Choose entrance object':'Review & export'} <Icon name="arrow" size={16}/></button>}
