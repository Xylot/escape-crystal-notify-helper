...
export default function App() {
  ...
  function select(b:Boss) {setView('editor');setActive(b.id);setStep(isDungeon(b)?'coverage':'setup');window.scrollTo({top:0});if(!b.maps.length&&!b.locationsLoaded&&!busy)void loadLocations(b);}
  ...
  async function sync(){await run('Syncing plugin',async()=>{
    ...
    setSnapshot(s=>({...s,source,entries,baseCommit:c.sha,generatedAt:new Date().toISOString(),warnings:[]}));setNotice(`Synced ${entries.filter(e=>['BOSSES','DUNGEONS'].includes(e.regionType??'')).length} boss and dungeon entries. Draft baselines were preserved.`);
  });}
  ...
  return <div className={`app-shell ${view==='editor'?'authoring-view':''}`}>
    ...
    {review&&<ReviewQueue drafts={changes} snapshot={snapshot} onClose={()=>setReview(false)} onEdit={id=>{const target=bosses.find(b=>b.id===id);if(target){select(target);setStep('review');setReview(false);}}} onExport={exportData} onCreatePR={prEnabled?openPR:undefined}/>}
