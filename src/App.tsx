import {withMetadata,METADATA_PATHS,metadataFor} from './core/encounter-metadata.mjs';
import {isDungeon} from './core/encounter-kind.mjs';
import {fetchDungeonCatalog,retainDungeonLocations} from './core/dungeons.mjs';
...
import { applyProposalFiles, validateProposal, proposalPatch, PLUGIN_REPO, JAVA_PATH } from './core/proposal.mjs';
...
export default function App() {
  ...
  const registerEditGuard=useCallback((guard:EditNavigationGuard)=>{editGuard.current=guard;},[]);
  ...
  const setNotice=useCallback((message:string)=>setNoticeState(message?{message}:null),[]);
  useEffect(()=>{
    if(!notice)return;
    const timer=window.setTimeout(()=>setNoticeState(null),5000);
    return()=>window.clearTimeout(timer);
  },[notice]);
  const [busy,setBusy]=useState(''),[modal,setModal]=useState(false),[review,setReview]=useState(false);
  ...
  useEffect(()=>{if(!PR_API)return;prJSON('/config').then(c=>{setPREnabled(c.enabled&&c.proposalVersion>=4);if(c.enabled&&!(c.proposalVersion>=4))setNotice('Update the PR backend to support instanced bosses, inactivity times, and pet icons. Proposal downloads remain available.');}).catch(()=>{});finishRedirect().then(login=>{if(login){const ids=JSON.parse(sessionStorage.getItem('escape-crystal-pr-selection')||'[]');if(ids.length)setPRIds(ids);}}).catch(e=>setNotice(e.message));},[]);
  ...
  useEffect(()=>{
    ...
    const listener=(event:KeyboardEvent)=>{
      if(event.key==='Escape'){setModal(false);setReview(false);setHelp(false);setPRIds(null);}
  ...
  useEffect(()=>{ (async()=>{
    try {
      ...
      if(raw) { const saved=JSON.parse(raw);if(saved.version!==1)throw new Error('Saved drafts use an unsupported format.');setDrafts(Object.fromEntries(Object.entries(saved.drafts??{}).map(([id,d])=>[id,defaultDraft(d)])));setImports(saved.imports??[]);if(saved.snapshot?.baseCommit && Date.parse(saved.snapshot.generatedAt??'')>Date.parse(initial.generatedAt??''))setSnapshot({...saved.snapshot,dungeons:saved.snapshot.dungeons??initial.dungeons,catalog:saved.snapshot.catalog??initial.catalog,candidates:saved.snapshot.catalog?saved.snapshot.candidates:initial.candidates}); }
  ...
  const bosses=useMemo(()=>resolveLibrary(drafts).map(b=>({...b,...withMetadata({...b,baseRaw:b.raw},snapshot.metadataSources)})) as Boss[],[resolveLibrary,drafts,snapshot.metadataSources]);
  const boss=bosses.find(b=>b.id===active)??bosses[0];
  const draft=useMemo(()=>boss?withMetadata(drafts[boss.id]??draftOf(boss),snapshot.metadataSources):null,[boss,drafts,snapshot.metadataSources]);
  ...
  async function sync(){await run('Syncing plugin',async()=>{
    ...
    const metadataSources=Object.fromEntries(await Promise.all(METADATA_PATHS.map(async path=>{const response=await fetch(`https://raw.githubusercontent.com/${PLUGIN_REPO}/${c.sha}/${path}`,{signal:AbortSignal.timeout(20000)});if(!response.ok)throw new Error('Could not load encounter metadata.');return [path,await response.text()];})));
    const entries=parseJava(source).entries.map(e=>({...e,maps:[],warnings:[],links:[],wikiTitle:e.name})) as Boss[];
    for(const entry of entries.filter(e=>['BOSSES','RAIDS'].includes(e.regionType??'')))metadataFor(entry.id,metadataSources);
    setSnapshot(s=>({...s,source,metadataSources,entries,baseCommit:c.sha,generatedAt:new Date().toISOString(),warnings:[]}));setNotice(`Synced ${entries.filter(e=>['BOSSES','RAIDS','DUNGEONS'].includes(e.regionType??'')).length} boss and dungeon entries. Draft baselines were preserved.`);
  });}
  async function discover(){await run('Loading wiki catalog',async()=>{
    const Parser=await loadParser(),[catalog,dungeonCatalog]=await Promise.all([fetchBossCatalog(Parser),fetchDungeonCatalog(Parser)]);
    setSnapshot(s=>({...s,dungeons:retainDungeonLocations(dungeonCatalog.dungeons,s.dungeons),candidates:catalog.bosses.map(b=>{const previous=s.candidates.find(p=>p.wikiTitle===b.wikiTitle);return {...b,maps:previous?.maps??[],links:previous?.links??[],locationsLoaded:previous?.locationsLoaded??false};}),catalog:{source:catalog.source,revision:catalog.revision,count:catalog.bosses.length,fetchedAt:new Date().toISOString()}}));
    setNotice(`Loaded ${catalog.bosses.length} bosses and ${dungeonCatalog.dungeons.length} dungeons from the wiki. Support is compared with the saved plugin source; Sync plugin to update that baseline.`);
  });}
  async function loadLocations(target:Boss=boss){if(!target)return;await run(`Loading ${target.name} locations`,async()=>{
    const result=await importWiki(await loadParser(),target.wikiTitle,target.locationTitles??[],!isDungeon(target));
  ...
  async function doImport(){await run('Reading wiki',async()=>{
    ...
    const result=paste.trim()?extractWiki(Parser,{title,text:paste,revision:null}):await importWiki(Parser,title,[],importKind!=='DUNGEONS');
    ...
    const existing=bosses.find(b=>isDungeon(b)===(importKind==='DUNGEONS')&&(b.wikiTitle.toLowerCase()===result.title.toLowerCase()||b.name.toLowerCase()===result.title.toLowerCase()));
  ...
  function proposal(ids?:string[]){const selected=ids?changes.filter(c=>ids.includes(c.id)):changes;const p={version:4,repository:PLUGIN_REPO,baseCommit:snapshot.baseCommit,changes:selected};validateProposal(p);if(conflicts.some(c=>selected.some(d=>d.id===c.id)))throw new Error('Resolve upstream conflicts before exporting.');return p;}
  function exportData(kind:string,ids?:string[]){try{
    const p=proposal(ids),sources={[JAVA_PATH]:snapshot.source,...snapshot.metadataSources},after=applyProposalFiles(sources,p);
    ...
    if(kind==='patch'){saveFile('escape-crystal.patch',proposalPatch(sources,after),'text/x-diff');setNotice('Patch downloaded.');}
