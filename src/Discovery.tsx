...
import { Icon, BossSigil } from './Icons';
...
type Props = {
  ...
  includeQuests: boolean; setIncludeQuests: (value: boolean) => void; onImport: () => void;
...
export default function Discovery(p: Props) {
  ...
  useEffect(() => setLimit(24), [p.search, p.filter, p.category, p.includeQuests, sort]);
  ...
  const reset = () => { p.setSearch(''); p.setCategory('all'); p.setFilter('all'); p.setIncludeQuests(true); };
  return <section className={`discovery ${p.filter === 'drafts' ? 'drafts-view' : ''}`} aria-label="Encounter library">
    ...
    <div id="encounters" className="encounter-library">
      ...
      <div className="library-toolbar"><label className="search"><Icon name="search"/><span className="sr-only">Search bosses</span><input type="search" placeholder="Search encounters…" value={p.search} onChange={e => p.setSearch(e.target.value)}/></label><label className="category-filter"><span className="sr-only">Boss category</span><select value={p.category} onChange={e => p.setCategory(e.target.value)}><option value="all">All categories</option>{p.categories.map(c => <option key={c}>{c}</option>)}</select></label><label className="sort-filter"><span className="sr-only">Sort encounters</span><select value={sort} onChange={e => setSort(e.target.value)}><option value="name">Name: A–Z</option><option value="locations">Locations first</option></select></label><div className="layout-switch" aria-label="Display layout"><button aria-label="Grid view" aria-pressed={layout === 'grid'} className={layout === 'grid' ? 'chosen' : ''} onClick={() => setLayout('grid')}><Icon name="grid" size={16}/></button><button aria-label="List view" aria-pressed={layout === 'list'} className={layout === 'list' ? 'chosen' : ''} onClick={() => setLayout('list')}><Icon name="list" size={17}/></button></div></div>
      <div className="results-meta"><span role="status">{p.ready ? `${p.bosses.length} encounter${p.bosses.length === 1 ? '' : 's'}` : 'Loading encounters…'}{p.search && ` matching “${p.search}”`}</span>{p.filter !== 'drafts' && <label className="quest-toggle"><input type="checkbox" checked={p.includeQuests} onChange={e => p.setIncludeQuests(e.target.checked)}/> Include quest bosses</label>}</div>
      <div className={`discovery-cards ${layout === 'list' ? 'list-layout' : ''}`}>{sorted.slice(0, limit).map(b => { const supported = !!b.raw || !!b.supportedBy?.length; const saved = p.drafts[b.id]; return <button className="discovery-card" key={b.id} onClick={() => p.onSelect(b)}><div className="card-top"><BossSigil category={b.categories?.[0]}/><span className={`encounter-status ${saved ? 'draft-status' : supported ? 'supported-status' : ''}`}><span className="dot"/>{saved ? 'Draft saved' : supported ? 'Supported' : 'Needs coverage'}</span></div><div className="card-copy"><span className="card-category">{b.categories?.[0] ?? 'Encounter'}</span><h3>{b.name}</h3></div><div className="card-bottom"><span><Icon name={b.maps.length ? 'map' : 'compass'} size={14}/>{b.maps.length ? `${b.maps.length} wiki locations` : 'Explore locations'}</span><Icon name="arrow" size={17}/></div></button>; })}</div>
      {p.ready && !p.bosses.length && <div className="library-empty"><Icon name={p.filter === 'drafts' ? 'file' : 'search'} size={34}/><h3>{p.filter === 'drafts' ? 'Your next contribution starts here.' : 'No encounters found.'}</h3><p>{p.filter === 'drafts' ? 'Choose an encounter and edit its coverage to save your first draft.' : 'Try another name, change category, or include quest bosses.'}</p><button onClick={reset}>Browse all encounters <Icon name="arrow" size={16}/></button></div>}
    ...
    </div><footer className="discovery-footer"><span><Icon name="spark" size={14}/> Every contribution makes Gielinor a little safer.</span><a href="https://oldschool.runescape.wiki/w/Boss" target="_blank" rel="noreferrer">Powered by OSRS Wiki <Icon name="external" size={12}/></a></footer>
  </section>;
}
