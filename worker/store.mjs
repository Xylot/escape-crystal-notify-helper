export class Store {
  ...
  async history(owner){const rows=await this.db.prepare('SELECT data FROM submissions WHERE owner=?').bind(owner).all();return rows.results.map(r=>JSON.parse(r.data)).filter(r=>r.pr);}
